  ...
  <div class="sidebar_box">
    ...
    <div id="templatemo_content">
      <div class="post_section">
                ...
                <?php 
				echo"<form action='arama.php' method='post'>
				Sınıf: <select name='ara_sinif_id'>
				...
				<option value='11'>12.Sınıf</option>
				<option value='12'>12.Sınıf</option>
				</select>";
				echo"ders:<select name='ara_ders_id'><option value=''>Ders Seç</option>";
				$sorgu="SELECT adlari.ders_adi, adlari.id FROM adlari ";
				$sonuc=mysql_query($sorgu);
				while($row=mysql_fetch_array($sonuc))
				{ extract($row);
				
					echo"<option value='$id'>$ders_adi</option>";
				} 
				echo"</select>
				<input type='submit' value='ARA' class='submitbutton buton' />
				</form>";
				
				 $sorgu="SELECT * FROM alt_konu WHERE ";
				 if($ara_sinif_id<>'')
				 {$sorgu.=" alt_konu.sinif_id = '$ara_sinif_id' and ";}
				 if($ara_ders_id<>'')
				 {$sorgu.=" alt_konu.ders_id = '$ara_ders_id' and ";}
				 $sorgu.="id>0 order by id desc";
				 $sonuc=mysql_query($sorgu);
				 ...
				 if($say<>'0')
				 {
					 ?>
					 <div id="a">
            			<div id="a1">Konu Başlığı</div>
            			<div id="a2">Konu Alt Başlığı</div>
            			<div id="a3">Detaylı Gör</div>
            		</div>
                    <?php
					 while($row=mysql_fetch_array($sonuc))
					 {
						 extract($row); 
						 echo"<form action='detay.php?sor_id=$id' method='post'>";
						 ?>
						 <div id="b">
							<div id="b1"><?php echo"$konu_basligi"; ?></div>
							<div id="b2"><?php echo"$konu_alt_basligi"; ?></div>
							<div id="b3"><?php echo"<input type='submit' value='Detaylı Gör' style='height:33px' class='buton'"; ?></div>
						 </div>
						 <?php 
					 echo"</form>";
					 }
